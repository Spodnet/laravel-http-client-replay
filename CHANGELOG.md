...
## [Unreleased](https://github.com/spodnet/laravel-http-client-replay/compare/v0.2.0...main)

## [v0.2.0](https://github.com/spodnet/laravel-http-client-replay/compare/v0.1.0...v0.2.0) - 2026-09-16

### Added

- Auto-expiring cassettes (TTL) support on read with Laravel Cache-aligned APIs:
  - Configure TTL globally, per-driver, or per-URL scope pattern (`'scopes' => ['pattern*' => ['ttl' => 3600]]`).
  - Request-level TTL overrides via `Http::withOptions(['replay_ttl' => 3600])` or `X-Replay-TTL` header.
  - Fluent cache-style helper methods on `HttpClientReplay`: `remember()`, `rememberForever()`, `ttl()`, `forever()`, `touch()`, `withTtl()`, and `scope()`.
  - New `CassetteExpired` event dispatched when a stale cassette is evicted on read.
  - Added `--expired` option to `php artisan replay:clear` command to purge only expired cassettes.
  - New testing assertions: `assertCassetteExpired()` and `assertCassetteNotExpired()`.
